...
9. 产品必须吸收腾讯公益数字化文章中的三个启发：AI问项目、钱/物/服资源匹配、受助人真实需要与尊严/选择权。
...
2. AI问项目：公众可在项目详情页询问当前最缺什么、为什么不能直接付款、自己的资源是否适合，AI 只基于脱敏项目事实和合规边界回答。
3. 求助申请入口：表单录入大病救助申请，包含受助人真实需要和“我不会整理材料，先写一段话”的低门槛输入，AI 输出结构化档案和缺失材料清单。
4. 机构四辨工作台：AI 输出辨善恶、辨真伪、辨大小、辨远近、人工复核 checklist 和决策按钮。
5. 捐助意向管理：公众提交钱/物/服三类帮助意向，AI 分类、匹配项目真实需要并生成机构跟进话术。
6. 透明反馈草稿：从项目进展生成需要机构复核的反馈文本。
...
- 右上角三个入口清晰可见。
- 项目详情页有 AI问项目。
- 求助申请入口能记录受助人真实需要和低门槛叙事输入。
- 捐助意向管理支持钱/物/服分类，并显示匹配到的真实需求。
